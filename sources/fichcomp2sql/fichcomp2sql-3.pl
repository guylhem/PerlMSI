...
unless ( scalar @ARGV == 3 ) {
    die "Usage:\n\t$0 fichcomp annee suffixe (mon ou dmi)\n";
}
...
while ( $line = <IFD> ) {
 if (length($line) > 2 ) { 
    $finess = mysubstr( $line, 0, 9 );
    $prestation = mysubstr( $line, 9, 2 );
    $iep    = mysubstr( $line, 13, 20 );
...
$dbh->do("DROP TABLE IF EXISTS fichcomp_$suffixe" . "_" . "$annee CASCADE; CREATE TABLE fichcomp_$suffixe" . "_" . "$annee (ligne_sq SERIAL PRIMARY KEY, finess bigint, prestation smallint, iep bigint, date timestamp, ucd int, qte int, prix int); select setval('fichcomp_" . $suffixe . "_" . $annee . "_ligne_sq_seq', 1)") or die "Ne peut initialiser fichcomp_$suffixe" . "_" . "$annee" . "_ligne_sq_seq!";
            $schema_sql = 1;
	    $dbh->commit;
        }
...
# debug
print "INSERT INTO fichcomp_$suffixe" . "_" . "$annee (finess, prestation, iep, date, ucd, qte, prix) VALUES ($finess, $prestation, $iep, $date, $ucd, $qte, $prix)\n";
if ($prix == "DEFAULT") { $prix=0;  }

$sth = $dbh->prepare("INSERT INTO fichcomp_$suffixe" . "_" . "$annee (finess, prestation, iep, date, ucd, qte, prix) VALUES (?, ?, ?, ?, ?, ?, ?)");

$sth->execute($finess, $prestation, $iep, $date, $ucd, $qte, $prix) or die "Impossible de rajouter $ligne_sq dans fichcomp_$suffixe" . "_" . "$annee!";
$sth->finish();
