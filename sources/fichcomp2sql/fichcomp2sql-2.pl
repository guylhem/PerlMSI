...
$dbh->do("DROP TABLE IF EXISTS fichcomp_$suffixe" . "_" . "$annee CASCADE; CREATE TABLE fichcomp_$suffixe" . "_" . "$annee (ligne_sq SERIAL PRIMARY KEY, finess bigint, prestation smallint, iep int, date timestamp, ucd int, qte int, prix int); select setval('fichcomp_" . $suffixe . "_" . $annee . "_ligne_sq_seq', 1)") or die "Ne peut initialiser fichcomp_$suffixe" . "_" . "$annee" . "_ligne_sq_seq!";
            $schema_sql = 1;
	    $dbh->commit;
        }
