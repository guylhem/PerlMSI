...
while ( $line = <IFD> ) {
 if (length($line) > 2 ) { 
    ...
    $iep    = mysubstr( $line, 22, 7 );
    $ucd   = mysubstr( $line, 29, 7 );
    $qte    = mysubstr( $line, 37, 9 );
        if ( $schema_sql != 1 ) {
# Supprimer de suite la table si elle existe
$dbh->do("DROP TABLE IF EXISTS fichcomp_mon_$annee CASCADE; CREATE TABLE fichcomp_mon_$annee (ligne_sq SERIAL PRIMARY KEY, finess bigint, iep int, ucd int, qte int); select setval('fichcomp_mon_" . $annee . "_ligne_sq_seq', 1)") or die "Ne peut initialiser fichcomp_mon_$annee" . "_ligne_sq_seq!";
            $schema_sql = 1;
	    $dbh->commit;
        }


$sth = $dbh->prepare("INSERT INTO fichcomp_mon_$annee (finess, iep, ucd, qte) VALUES (?, ?, ?, ?)");

$sth->execute($finess, $iep, $ucd, $qte) or die "Impossible de rajouter $ligne_sq dans fichcomp_mon_$annee!";
$sth->finish();
