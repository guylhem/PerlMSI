...
# if length
 ...
 if ($lines_missing) {
 printf STDERR $lines_missing . " lignes non lues\n"
 } # if lines_missing

# Store metadata as direct hash keys
  $$E_result{NBR}=$line_cur;
  $$E_result{DISTINCT_IEP}=scalar keys %iep;
  $$E_result{DISTINCT_SS}=scalar keys %ss;
...
sub distinct_iep {
  my $E_self = shift;
  return $$E_self{DISTINCT_IEP};
 }
# sub distinct_iep

sub distinct_ss {
  my $E_self = shift;
  return $$E_self{DISTINCT_SS};
 }
# sub distinct_ss
...
my $line_cur = 0;
while ( my $line_read = <IFD> ) {
 # Can't use line lengh. Must do partial read.
...
my ($iep_read, $dasnbr_read, $actesnbr_read, @das_read)=
...
$RESERVE2[$line_cur], 
$DAS[$line_cur]) =
unpack('A2 A2 A4 A1 A3 A3 A9 A3 A20 A20 A10 A8 A1 A4 A2 A8 A1 A1 A8 A1 A1 A5 A4 A2 A2 A2 A2 A3 A8 A8 A3 A1 A1 A1 A11 @125 A2 @165/(A8)', $line_read);
...
$jump=165+$DAS_NBR[$line_cur]*8;
@{ $DAD[$line_cur] } = unpack('x' .$jump . '(A8)[' .  $DAD_NBR[$line_cur] . ']', $line_read);
$jump=165+$DAS_NBR[$line_cur]*8+$DAD_NBR[$line_cur]*8;
# Each line is a block of 26 char to be converted into a proper hash
my (@actesraw_read) = unpack('x' .$jump . '(A26)[' .  $ACTES_NBR[$line_cur] . ']', $line_read);

my $actesraw_nbr=scalar(@actesraw_read);
my $actesraw_cur=0;
while ($actesraw_cur < $actesraw_nbr) {
 ($CCAM[$line_cur][$actesraw_cur],
 $PHASE[$line_cur][$actesraw_cur],
 $ACTIVITE[$line_cur][$actesraw_cur],
 $EXTENSION[$line_cur][$actesraw_cur],
 $MODIFICATEUR[$line_cur][$actesraw_cur],
 $REMB_EXCEPTIONNEL[$line_cur][$actesraw_cur],
 $ASSOC_NONPREVUE[$line_cur][$actesraw_cur],
 $ITERATION[$line_cur][$actesraw_cur])=unpack('A7 A1 A1 A1 A4 A1 A1 A2',
$actesraw_read[$actesraw_cur]);
...
### Store metadata as direct hash keys
  $$E_result{NBR}=$line_cur;
 $$E_result{DISTINCT_IEP}=scalar keys %iep;
 $$E_result{DISTINCT_RSS}=scalar keys %rss;
...
$ACTES{CCAM}=\@CCAM;
$ACTES{PHASE}=\@PHASE;
$ACTES{ACTIVITE}=\@ACTIVITE;
$ACTES{EXTENSION}=\@EXTENSION;
$ACTES{MODIFICATEUR}=\@MODIFICATEUR;
$ACTES{REMB_EXCEPTIONNEL}=\@REMB_EXCEPTIONNEL;
$ACTES{ASSOC_NONPREVUE}=\@ASSOC_NONPREVUE;
$ACTES{ITERATION}=\@ITERATION;
$$E_result{ACTES}=\%ACTES;
...
sub sceances {
  ...
  while ($line_cur < $line_nbr) {
   if ($$E_self{SCEANCES_NBR}[$line_cur]) {
    if ($$E_self{SCEANCES_NBR}[$line_cur]>0) {
    my $uf=$$E_self{UF}[$line_cur];
...
my $E_rss2009=RSS::readfrom("2009-M8-2e.rss");
