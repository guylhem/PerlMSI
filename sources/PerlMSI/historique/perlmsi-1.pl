...
$jump=$DAS_NBR[$line_cur]*8;
($discard, @{ $DAD[$line_cur] }) = unpack('A' . $jump . ' (A8)[' .  $DAD_NBR[$line_cur] . ']', $dasdadactesraw_read);
$jump=$DAS_NBR[$line_cur]*8+$DAD_NBR[$line_cur]*8;
# Each line is a block of 26 char to be converted into a proper hash
($discard, @actesraw_read) = unpack('A' . $jump . ' (A26)[' .  $ACTES_NBR[$line_cur] . ']', $dasdadactesraw_read);
...
sub sceances {
  ...
  while ($line_cur < $line_nbr) {
   if ($$E_self{SCEANCES_NBR}[$line_cur]) {
    ...
    $I_sc{$uf}+=$$E_self{SCEANCES_NBR}[$line_cur];
    } # if
   }
   $line_cur++;
  } #while
...
my %sceances_stats=$E_rss2009->sceances;


#print Dumper %sceances_stats;
