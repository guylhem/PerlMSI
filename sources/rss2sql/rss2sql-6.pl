...
"INSERT into transmis (annee, version_groupage, cmd, ghm, filler, version_rss, code_retour, finess, version_rum, rss, iep, ddn, sexe, uf, uf_autorisation, lit_autorisation, reserve1, date_entree_uf, entree, provenance, date_sortie_uf, sortie, destination, cp_residence, poids_nne_entree, sceances_nbr, das_nbr, dad_nbr, actes_nbr, dp, dr, igs2, reserve2) values ($annee, $version_groupage, '$cmd', '$ghm', $filler, $version_rss, $code_retour, '$finess', $version_rum, $rss, $iep, '$ddn', '$sexe', $uf, $uf_autorisation, $lit_autorisation, $reserve1, '$date_entree_uf', $entree, $provenance, '$date_sortie_uf', $sortie, $destination, $cp_residence, $poids_nne_entree, $sceances_nbr, $das_nbr, $dad_nbr, $actes_nbr, '$dp', '$dr', $igs2, $reserve2);";
        ...
        if ( $das_nbr > 0 ) {
            ...
            while ( $das_cur < $das_nbr ) {
                $sql =
"INSERT INTO rss_diags (code_sq_fk, rss, iep, das) VALUES (currval('rss_code_sq_pk_seq'), $rss, $iep, '$das[$das_cur]');";
                print $sql . "\n";
                $sql =
"select currval('rss_code_sq_pk_seq') as rss_diag_das;";
                print $sql . "\n";
                $das_cur++;
            }    # while das_cur
        }    # das_nbr
        ...
        if ( $dad_nbr > 0 ) {
            ...
            while ( $dad_cur < $dad_nbr ) {
                $sql =
"INSERT INTO rss_diags (code_sq_fk, rss, iep, dad) VALUES (currval('rss_code_sq_pk_seq'), $rss, $iep, '$dad[$dad_cur]');";
                print $sql . "\n";
                $sql =
"select currval('rss_code_sq_pk_seq') as rss_diags_dad;";
                print $sql . "\n";
                $dad_cur++;
            }    # while dad_cur
        }    # dad_nbr
