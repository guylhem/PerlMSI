...
where evaluation_tra_$suffixe.rsa_sq=evaluation_ghsrsa_$suffixe.rsa and ghm_in=g1.ghm and t1.ghs=evaluation_ghsrsa_$suffixe.ghs and t1.datefin>date(annee_sortie||' '||mois_sortie|| ' 01') and t1.datedebut<=date(annee_sortie||' '||mois_sortie|| ' 01')";
...
$gagne_val=q0("select cast (sum(valo_tot_$suffixe-valo_tot_reference) as int) from evaluation_delta_reference_$suffixe where valo_tot_nodas>valo_tot_reference");
$perd_nbr=q0("select count (*) from evaluation_delta_reference_$suffixe where valo_tot_$suffixe < valo_tot_reference");
$perd_val=q0("select cast (sum(valo_tot_$suffixe-valo_tot_reference) as int) from evaluation_delta_reference_$suffixe where valo_tot_nodas<valo_tot_reference");
