...
# Tous les diags_s déclarés : attention, il peut y en avoir des NULLS (!!!)
my $sth = $dbh->prepare(<<SQL);
select distinct das from (select das from transmis_diags_2007 where iep in (select iall.iep from transmis_2007, ipp_iep i07, ipp_iep iall where transmis_2007.code_sq_pk='$code_sq_pk' and i07.iep=transmis_2007.iep and iall.ipp=i07.ipp and transmis_2007.iep not in (select iep from contentieux_secu_2007)) union all select das from transmis_diags_2006 where iep in (select iall.iep from transmis_2007, ipp_iep i07, ipp_iep iall where transmis_2007.code_sq_pk='$code_sq_pk' and i07.iep=transmis_2007.iep and iall.ipp=i07.ipp and transmis_2007.iep not in (select iep from contentieux_secu_2007)) union all select das from transmis_diags_2005 where iep in (select iall.iep from transmis_2007, ipp_iep i07, ipp_iep iall where transmis_2007.code_sq_pk='$code_sq_pk' and i07.iep=transmis_2007.iep and iall.ipp=i07.ipp and transmis_2007.iep not in (select iep from contentieux_secu_2007))) as toto where das is not null;
SQL

#select das
#   from transmis_diags_2007 
#   where code_sq_fk='$code_sq_pk'

$sth->execute or die "Impossible de faire la requete sur transmis_diags pour das!";
...
$sth->execute or die "Impossible de faire la requete sur transmis_diags pour dad!";
...
# select min(date_acte) as acte_date,code_ccam as acte_code_ccam, phase as acte_phase, activite as acte_activite, ext_doc as acte_ext_doc, modificateur as acte_modificateur, remb_exceptionnel as acte_remb_exceptionnel, assoc_nonprevue as acte_assoc_nonprevue, case when count(*)>99 then 99 else count(*) end as iteration from transmis_actes_2007   where code_sq_fk=40725 group by code_sq_fk, code_ccam, phase, activite, ext_doc, modificateur, remb_exceptionnel, assoc_nonprevue
# case when count>100 then 99 else end

# Tous les actes déclarés + les non déclarés ssi pas déjà déclarés et dans premier RUM...
# Bug: si date_sortie_uf antérieure à date d'acte (multirum), marche pas...
my $sth = $dbh->prepare(<<SQL);
   select min(date_acte) as acte_date,code_ccam as acte_code_ccam, phase as acte_phase, activite as acte_activite, ext_doc as acte_ext_doc, modificateur as acte_modificateur, remb_exceptionnel as acte_remb_exceptionnel, assoc_nonprevue as acte_assoc_nonprevue, case when count(*)>99 then 99 else count(*) end as iteration
   from transmis_actes_2007 
   where code_sq_fk='$code_sq_pk'
   group by code_sq_fk, code_ccam, phase, activite, ext_doc, modificateur, remb_exceptionnel, assoc_nonprevue
   UNION ALL
   select distinct on (acte) date_arrivee as acte_date, acte as acte_code_ccam, 0 as acte_phase, 1 as acte_activite, null as acte_ext_doc, 'DEFAULT' as acte_modificateur, 'DEFAULT' as acte_remb_exceptionnel, null as acte_assoc_nonprevue, 1 as acte_iteration from lambda_07_actes_rx, transmis_actes_2007, transmis_2007
   where  transmis_2007.code_sq_pk=transmis_actes_2007.code_sq_fk and lambda_07_actes_rx.iep='$iep' and  lambda_07_actes_rx.iep=transmis_actes_2007.iep and lambda_07_actes_rx.acte!=transmis_actes_2007.code_ccam and
   lambda_07_actes_rx.date_arrivee <= transmis_2007.date_sortie_uf and lambda_07_actes_rx.date_arrivee>transmis_2007.date_entree_uf
   group by lambda_07_actes_rx.acte, lambda_07_actes_rx.date_arrivee
SQL
# transmis_actes_2007.code_sq_fk in (select distinct code_sq_fk from transmis_actes_2007 where iep='$iep' order by code_sq_fk limit 1)
...
$sth->execute or die "Impossible de faire la requete sur transmis_actes";
...
print myoutstrnuml($code_retour, 0, 3);
...
print myoutstrnuml($diags_das_nbr, 0, 2);
