...
$schema_sql=1;
		...
		$rsa=mysubstr($rsa_line, 13, 10);
		$version_rss=mysubstr($rsa_line, 23, 3);
		$sequence_tarif=mysubstr($rsa_line, 26, 3);
		$version_lu=mysubstr($rsa_line, 29, 2);
		$ghm_lu=mysubstr($rsa_line, 31, 6);
		$retour_lu=mysubstr($rsa_line, 37, 3);
		$version_mis=mysubstr($rsa_line, 40, 2);
		$ghm_mis=mysubstr($rsa_line, 42, 6);
		$retour_mis=mysubstr($rsa_line, 48, 3);
		$rum_nbr=mysubstr($rsa_line, 51, 2);
		$age_annee=mysubstr($rsa_line, 53, 3);
		$age_jour=mysubstr($rsa_line, 56, 3);
		$sexe=mysubstr($rsa_line, 59, 1);
		$entree=mysubstr($rsa_line, 60, 1);
		$provenance=mysubstr($rsa_line, 61, 1);
		$mois_sortie=mysubstr($rsa_line, 62, 2);
		$annee_sortie=mysubstr($rsa_line, 64, 4);
		$sortie=mysubstr($rsa_line, 68, 1);
		$destination=mysubstr($rsa_line, 69, 1);
		$type=mysubstr($rsa_line, 70, 1);
		$duree=mysubstr($rsa_line, 71, 4);
		$cp_residence=mysubstr($rsa_line, 75, 5);
		$poids_nne=mysubstr($rsa_line, 80, 4);
		$nbr_sceances=mysubstr($rsa_line, 84, 2);
		$igs2=mysubstr($rsa_line, 86, 3);
		$ghs=mysubstr($rsa_line, 89, 4);
		$depassement_bornehaute=mysubstr($rsa_line, 93, 4);
		$inferieur_bornebasse=mysubstr($rsa_line, 97, 1);
		$ghs_forfait_dialyse=mysubstr($rsa_line, 98, 4);
		$supplement_hemodialyse_hs=mysubstr($rsa_line, 102, 3);
		$supplement_entraitement_dialyseperit_auto_hs=mysubstr($rsa_line, 105, 3);
		$supplement_entraitement_dialyseperit_ambu_hs=mysubstr($rsa_line, 108, 3);
		$supplement_entraitement_hemodialyse_hs=mysubstr($rsa_line, 111, 3);
		$sceances_avant_sros=mysubstr($rsa_line, 114, 2);
		$nbr_actes_ghm_24Z05Z_ou_28Z11Z=mysubstr($rsa_line, 116, 3);
 		$nbr_actes_ghm_24Z06Z_ou_28Z12Z=mysubstr($rsa_line, 119, 3);
 		$nbr_actes_ghm_24Z07Z_ou_28Z13Z=mysubstr($rsa_line, 122, 3);
 		$supplement_caisson_hyperbare=mysubstr($rsa_line, 125, 3);
 		$type_prelevement_organe=mysubstr($rsa_line, 128, 1);
 		$supplement_sra=mysubstr($rsa_line, 129, 3);
 		$supplement_rea=mysubstr($rsa_line, 132, 3);
 		$supplement_si_de_rea=mysubstr($rsa_line, 135, 3);
 		$supplement_stf=mysubstr($rsa_line, 138, 3);
 		$supplement_ssc=mysubstr($rsa_line, 141, 3);
 		$supplement_src=mysubstr($rsa_line, 144, 3);
 		$supplement_nn1=mysubstr($rsa_line, 147, 3);
 		$supplement_nn2=mysubstr($rsa_line, 150, 3);
 		$supplement_nn3=mysubstr($rsa_line, 153, 3);
 		$supplement_rep=mysubstr($rsa_line, 156, 3);
 		$lit_dedie_palliatif=mysubstr($rsa_line, 159, 1);
 		$dp=mysubstr($rsa_line, 160, 6);
 		$dr=mysubstr($rsa_line, 166, 6);
		$nb_diags=mysubstr($rsa_line, 172, 2);
		$nb_actes=mysubstr($rsa_line, 174, 4);
	...
	# Maintenant on parse le rsa_line selon le nbr DAS/DAD/ACTES
		...
		if ($das_nbr > 0) {
			...
			while ($das_cur < $das_nbr) {
				$das[$dad_cur]=mysubstr($rsa_line, $offset, 6);
				$offset=$offset+6;
				$das_cur++;
			} # while das_cur
		} # das_nbr
...
print $sql . "\n";
	...
	if ($rum_nbr > 0) {
		$rum_cur=0;
		while ($rum_cur < $rum_nbr) {
			$sql = "INSERT INTO rsa_rum_$annee (code_sq_fk, rsa_f, type_rum, duree_uf, valorisation_rea, valorisation_partielle, position_dp) VALUES (currval('rsa_code_sq_pk_seq'), $rsa, '$rum_type_rum[$rum_cur]', $rum_duree_uf[$rum_cur], $rum_valorisation_rea[$rum_cur], $rum_valorisation_partielle[$rum_cur], '$rum_position_dp[$rum_cur]');"; 
print $sql . "\n";
$sql="select currval('rsa_code_sq_pk_seq') as rsa_rum;";
print $sql . "\n";
			$rum_cur++;
		} # while rum_cur
	} # rum_nbr
	...
	if ($das_nbr > 0) {
		$das_cur=0;
		while ($das_cur < $das_nbr) {
			$sql = "INSERT INTO rsa_diags_$annee (code_sq_fk, rsa_f, das) VALUES (currval('rsa_code_sq_pk_seq'), $rsa, $das[$das_cur]');"; 
print $sql . "\n";
$sql="select currval('rsa_code_sq_pk_seq') as rsa_diags;";
print $sql . "\n";
			$das_cur++;
		} # while das_cur
	} # das_nbr
...
$sql= "INSERT INTO rsa_actes_$annee ( code_sq_fk, rsa_f, delai, code_ccam, phase, activite, ext_doc, modificateur, remb_exceptionnel, assoc_nonprevue, iteration) VALUES ( currval('rsa_code_sq_pk_seq'), $rsa, '$acte_date[$actes_cur]', '$acte_code_ccam[$actes_cur]', $acte_phase[$actes_cur], $acte_activite[$actes_cur], $acte_ext_doc[$actes_cur], '$acte_modificateur[$actes_cur]', '$acte_remb_exceptionnel[$actes_cur]', $acte_assoc_nonprevue[$actes_cur], $acte_iteration[$actes_cur]);";
print $sql . "\n";
