...
$line_cur=0;
	...
	if ($version_rsa == 214) {
		if ($schema_sql != 1) {
print "drop table national_$annee cascade;
drop table national_diags_$annee cascade;
drop table national_actes_$annee cascade;
drop table national_rum_$annee cascade;

create table national_$annee (
...
create table national_rum_$annee (
 code_sq_fk INT,
 rsa_f bigint,
 type_rum smallint,
 duree_uf smallint,
 ...
 position_dp char(2),
 FOREIGN KEY (code_sq_fk) REFERENCES national_$annee (code_sq_pk) ON DELETE CASCADE ON UPDATE CASCADE
);

create table national_diags_$annee (
 code_sq_fk INT,
 rsa_f bigint,
 das char(9),
 FOREIGN KEY (code_sq_fk) REFERENCES national_$annee (code_sq_pk) ON DELETE CASCADE ON UPDATE CASCADE
...
create table national_actes_$annee (
 code_sq_fk INT,
 rsa_f bigint,
 delai smallint,
 code_ccam char(8),
...
$sql="select currval('national_" . $annee . "_code_sq_pk_seq') as national;";
print $sql . "\n";

	if ($rum_nbr > 0) {
		$rum_cur=0;
		while ($rum_cur < $rum_nbr) {
			$sql = "INSERT INTO national_rum_$annee (code_sq_fk, rsa_f, type_rum, duree_uf, valorisation_rea, valorisation_partielle, position_dp) VALUES (currval('national_" . $annee . "_code_sq_pk_seq'), $rsa, $rum_type_rum[$rum_cur], $rum_duree_uf[$rum_cur], $rum_valorisation_rea[$rum_cur], $rum_valorisation_partielle[$rum_cur], '$rum_position_dp[$rum_cur]');"; 
print $sql . "\n";
$sql="select currval('national_" . $annee . "_code_sq_pk_seq') as national_rum;";
print $sql . "\n";
			$rum_cur++;
		} # while rum_cur
	} # rum_nbr
	...
	if ($das_nbr > 0) {
		$das_cur=0;
		while ($das_cur < $das_nbr) {
			$sql = "INSERT INTO national_diags_$annee (code_sq_fk, rsa_f, das) VALUES (currval('national_" . $annee . "_code_sq_pk_seq'), $rsa, '$das[$das_cur]');"; 
print $sql . "\n";
$sql="select currval('national_" . $annee . "_code_sq_pk_seq') as national_diags;";
print $sql . "\n";
			$das_cur++;
		} # while das_cur
	} # das_nbr
...
$sql= "INSERT INTO national_actes_$annee ( code_sq_fk, rsa_f, delai, code_ccam, phase, activite, ext_doc, modificateur, remb_exceptionnel, assoc_nonprevue, iteration) VALUES ( currval('national_" . $annee . "_code_sq_pk_seq'), $rsa, $acte_delai[$actes_cur], '$acte_code_ccam[$actes_cur]', $acte_phase[$actes_cur], $acte_activite[$actes_cur], '$acte_ext_doc[$actes_cur]', '$acte_modificateur[$actes_cur]', '$acte_remb_exceptionnel[$actes_cur]', '$acte_assoc_nonprevue[$actes_cur]', $acte_iteration[$actes_cur]);";
print $sql . "\n";
$sql="select currval('national_" . $annee . "_code_sq_pk_seq') as national_acte;";
print $sql . "\n";
